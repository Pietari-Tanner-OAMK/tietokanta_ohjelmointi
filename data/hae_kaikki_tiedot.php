<?php
    ...
    $avg= $_GET['avg'];
    //Select lause
    $sql = "SELECT primary_title, num_votes, genre, name_, job_category
    ...
    LIMIT 20;";
          ...
          foreach($rows as $row){
              //Tulostaa
            $html .= '<li>' . '<h3>' . $row['primary_title'] . '</h3>' . '<b> ' . 'Ääniä annettu: ' . '</b>' . $row['num_votes'] . '</br>'
             . '<b>' . 'Genre: ' . '</b>' . $row['genre'] . '<br>'  . '<b>' . $row['job_category'] . ': ' . '</b>' . $row['name_'] . '</br>' .  '</li>';
          }
          $html .= '</ul>';
          echo $html;